...
| Partial rounds                                                            | Hold them out as `Round Incomplete` at Round granularity on every comparison item. At Measure Standard and Measure granularity, keep scoring each measure the student sat (confirmed 2026-09-24) | Partly built. `aimline_round_category` already holds out all 1,115 AY2025 partial student-rounds. Still scoring them: `round_benchmark_status` and `round_trajectory_status` (all 1,115), and `pm_round_status`, which reads `Below Aimline` on 640 where a sat measure was below |

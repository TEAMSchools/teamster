...
Small details that silently corrupt a workbook or a diff.
...
no `Æ` sentinel. See [02-dynamic-text.md](02-dynamic-text.md).
...
string it returns contains no `\r` at all. This means:
...
lines inserted or removed, and bare-LF count should be zero.
...
inside the `.twbx` was not.

## Number formats

**Verified.** Formats live on the `<column>` definition as `default-format`, and
apply everywhere the field is used — labels, tooltips, headers — with no
per-sheet work.
...
to be done in Desktop, which is three clicks.

If the skill needs this, capture a Desktop-authored example first.
...
- A blanket `value='true'` → `value='false'` rewrote a parameter's domain
...
workbook truncated from 1.45M characters to 143k.

Cheap guard, worth having in every edit script:

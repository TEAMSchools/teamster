...
Every failure observed, as symptom to cause to fix. Ordered by where you notice
it. All entries are verified unless marked otherwise.
...
All six are covered by `scripts/check_twb.py`. Run it before handing anything
over; the whole point is to catch these without a Desktop round trip.
...
| A line in a mark label is blank, space still reserved      | Parameter token in `<customized-label>` — does not resolve                   | Use static text, or move the value to a worksheet title         |
...
| Tooltip prints raw `[federated…].[usr:Calculation_…:qk]`   | Encoding form — **unresolved**, see [02-dynamic-text.md](02-dynamic-text.md) | Copy a working tooltip's runs verbatim, swap only the instances |
...
| Two zones overlap on screen             | A zone at the wrong nesting depth — valid XML, no error | `scripts/check_geometry.py`                            |
...
drew no marks, including as a plain `Bar`. The approach was abandoned. If the
skill needs dual axis, budget a real investigation.
...
the axis to 200% doing it. Five variations were tried.

It works normally on a plain `usr:` axis:
...
That last one is worth spelling out, because it is a reasoning failure rather
than a mechanical one. On one panel the bar length resolved through a
parameter-aware calculation while the mark label and the colour resolved through
projected-only calculations. Setting the control to the other basis moved the
bars and left the labels, producing a near-zero bar labelled `+11.4pp` in green.

The analysis that missed it traced only the bar. **Trace every encoding on a
sheet — rows, columns, text, colour, size, tooltip — not just the one that looks
like the measure.**
...
against it. Make that a step, not an afterthought — `scripts/mutate.py`.

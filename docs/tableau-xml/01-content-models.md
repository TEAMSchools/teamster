...
Server does not. Every rule below was learned from a Desktop refusal on a file
that Server had already accepted and rendered.

Desktop's error text is the authoritative source, and it quotes the model
...
### Never reconstruct the manifest
...
Insert into the manifest. Never regenerate it. `scripts/check_twb.py` diffs
manifest entries against a reference workbook, including dotted names, for
exactly this reason.

## The content models

Copy these into any script that inserts elements. Order is strict; optionality
is marked as Desktop reports it.
...
to open the workbook. On sheets with a reference line — a goal tick, a target
marker — the tooltip must come after it.

The trap is that most sheets have no reference line, so an insert anchored on
`</encodings>` works everywhere until it doesn't.
...
- Every worksheet needs `<simple-id>`. Hand-built worksheets omit it easily and
  Desktop reports `missing elements in content model`.
...
omits it is rejected. The cheapest fix is to clone the filter/slices/aggregation
skeleton from a working sheet rather than compose one.
...
Getting this backwards is rejected. Indentation in the files observed is 14
spaces for a container's own style and 16 for its children's, though indentation
is cosmetic.
...
This is the single most dangerous class of edit, because the whole toolchain
reports success. `scripts/check_geometry.py` exists to cover it: it compares
each container's parent-minus-children gap against the same container in a
known-good baseline and requires an exact match.
...
`scripts/check_twb.py` does this as a post-hoc check; doing it at insert time as
well is cheap.

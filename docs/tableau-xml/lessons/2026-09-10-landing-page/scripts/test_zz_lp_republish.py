"""Throwaway (2026-09-10 render-fix round, delete after use): publish the
patched Landing Page workbook over the existing ZZ-REVIEW copy in
GPA-monitor-temp and render it.

Base is production revision 26, which already contains the landing page, so
`added` is still {"Landing Page"}: the dashboard exists in the file but is not
one of production's five live views.

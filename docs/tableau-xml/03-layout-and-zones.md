...
Everything here concerns that tree and how text behaves inside it.
...
per-container constant — margins — with **no relation to the number of
children**.
...
which have no baseline counterpart, get an absolute bound. That is what
`scripts/check_geometry.py` does.
...
Across two production dashboards, 62 zones with `is-fixed='true'`, the split was
21 exact and 41 with `fixed-size` smaller. Zero larger.
...
truncated with an ellipsis. It does not wrap to a second line.

The budget scales with the text zone's width. Calibration points, all measured
from renders:
...
- **A big number rendered as `####`.** Tableau's "does not fit" glyph. A mark
  label with three lines — 13pt caption, 10pt qualifier, 30pt number — did not
  fit a 66px zone, nor 78px, nor 88px. The fix was removing the third line, not
  growing the box.
...
Shortening the label text was cheaper than fighting the header sizing. The
labels are constant-string calculated fields, so it is a one-string edit.
...
partition, and Detail changes that partition.

Put tooltip fields on the **Tooltip** shelf. Reserve Detail for cases where the
grain change is the intent.

## Copying a card pattern

The layout idiom used across this suite, if the skill needs a default:
...
Two structural points worth carrying into the skill:

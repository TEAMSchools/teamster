...
| `race_ethnicity`      | Race and ethnicity as recorded in the student information system.         | `int_extracts__student_enrollments.race_ethnicity` | Asian, BL-AA, Hispanic or Latino, AI-AN, NH-OPI, 2+ races, White, DTS. DTS means declined to state. Blank when not matched.                                                                    |
| `gender`              | Gender as recorded in the student information system.                     | `int_extracts__student_enrollments.gender`         | Female, Male, Non-Binary. Blank when not matched.                                                                                                                                              |
| `meal_status`         | National School Lunch Program eligibility for the school year.            | `int_extracts__student_enrollments.lunch_status`   | Free, Reduced, Paid. Direct certification is reported as Free. Blank when not matched.                                                                                                         |
| `iep_status`          | Whether the student had an active individualized education program.       | `int_extracts__student_enrollments.iep_status`     | Has IEP, No IEP. Blank when not matched.                                                                                                                                                       |
| `mll_status`          | Multilingual learner status.                                              | `int_extracts__student_enrollments.ml_status`      | ML, Not ML. Blank when not matched.                                                                                                                                                            |

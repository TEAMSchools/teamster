...
## Addendum: Task 3 (added mid-execution)

After Tasks 1 and 2 were implemented and reviewed, the user requested two more
passthrough columns on `int_iready__domain_unpivot`: `_dbt_source_project` and
`test_round`. Both already exist on `int_iready__diagnostic_results` (the
model's only upstream ref) — `test_round` is
`coalesce(rt.name, 'Outside Round')` from a terms lookup, and
`_dbt_source_project` is the standard kipptaf cross-region join key, derived
once at the union view and passed through everywhere downstream (per
`src/dbt/kipptaf/CLAUDE.md` → "`_dbt_source_project` is pass-through, derived
only at the union view"). This task adds both as plain passthrough columns — no
new derivation, no join, no change to the UNPIVOT's domain tuples or to
`rn_subject_test`'s partition (both new columns are functionally determined by
columns already in that partition — `test_round` by `completion_date`,
`_dbt_source_project` by `_dbt_source_relation` — so adding them to the
partition would be redundant).

### Task 3: Pass through `_dbt_source_project` and `test_round`
...
- Consumes: `int_iready__diagnostic_results.test_round` (string) and
  `int_iready__diagnostic_results._dbt_source_project` (string) — both already
  produced by that model, no changes needed there.
- Produces: `int_iready__domain_unpivot` gains two columns, `test_round`
  (string) and `_dbt_source_project` (string), positioned after
  `completion_date` and before `domain_name` in the column list. All other
  columns and the grain are unchanged from Tasks 1-2.

- [ ] **Step 1: Extend the existing unit test with the two new columns (RED)**
  ...
  unit_tests:
    - name: unit_iready_domain_unpivot_placement_scale_score
      description:
        ...
        tuple is null, matching the prior single-column unpivot's behavior of
        excluding an unmeasured domain rather than emitting a row of nulls for
        it. Also verifies test_round and _dbt_source_project pass through
        unchanged onto every unpivoted domain row.
      model: int_iready__domain_unpivot
      given:
        - input: ref('int_iready__diagnostic_results')
          format: sql
...
- [ ] **Step 2: Run the unit test and confirm it fails**
  ...
  Expected: FAIL. The model doesn't yet select `test_round` or
  `_dbt_source_project`, so the actual output is missing columns the `expect`
  rows require.

- [ ] **Step 3: Add the two passthrough columns to the model**
...
- [ ] **Step 5: Add column docs to `properties.yml`**

  In the same file, replace the `models:` block with:
    ...
    - name: int_iready__domain_unpivot
      ...
      columns:
        ...
        - name: test_round
          data_type: string
          description:
            i-Ready testing round (BOY, MOY, EOY) the administration falls
            within, or Outside Round when it doesn't match any of the school's
            defined windows.
...
- [ ] **Step 6: Re-run the unit test after the properties change**
...
- [ ] **Step 8: Commit**
  ...
  git -C /workspaces/teamster/.worktrees/anthonygwalters/feat/claude-iready-domain-unpivot-placement-scale-score add src/dbt/kipptaf/models/iready/intermediate/int_iready__domain_unpivot.sql src/dbt/kipptaf/models/iready/intermediate/properties/int_iready__domain_unpivot.yml
  git -C /workspaces/teamster/.worktrees/anthonygwalters/feat/claude-iready-domain-unpivot-placement-scale-score commit -m "$(cat <<'EOF'
  feat(dbt): pass through _dbt_source_project and test_round in int_iready__domain_unpivot

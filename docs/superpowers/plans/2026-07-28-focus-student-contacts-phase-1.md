...
pickup/dropoff). The `phone_type` / `is_email_title` mapping shown inline above
(the `phones` CTE and its regex `case`) was later moved into
`stg_focus__people_join_contacts` staging, and `contact_name` into
`stg_focus__people` staging — both are consumed pre-derived here instead of
being computed in this model. A further round removed the five
`finalsite.clean_phone(...)` wraps on the phone columns (owner decision: the
`focus` package must not reference a `finalsite` macro) — phone values are now
emitted raw, and E.164 normalization moves to the kipptaf consumer in Phase 2.
That column-shape change also triggered the ST06 re-ordering above: the phone
columns are now plain column references, so they moved into the plain-ref block
grouped by source table in join order (`l` → `s` → `p` → `pt` → `pp` → `a`),
pushing `home_address` after the phone columns.

...
| `dim_staff_work_assignments.time_approver_staff_key`                                                                                                                                                                              | not present                                                                                                                                                                                                                                                      | `generate_surrogate_key(["sup.employee_number"])` via second self-join on `stg_people__employee_numbers`, null-guarded. Renamed from `time_service_supervisor_staff_key` in review followup — cold-read test showed "time_service" prefix read as opaque ADP jargon.                                                                                                                              | Structural add                                                                                                                      |
...
| `fct_grades_gpa.term_key`                                                                                                                                                                                                         | `generate_surrogate_key(["rt.type", "rt.code", "rt.name", "rt.start_date", "rt.region", "rt.school_id"])` joined on a broken predicate (`rt.code` vs. PS `Q#` names, spurious `yearid - 1990` offset) — produced the all-NULLs placeholder hash for 100% of rows | Same derivation, join fixed to `gt.term_name = rt.``name`` AND gt.schoolid = rt.school_id AND gt.yearid = rt.powerschool_year_id` with `reporting_terms` CTE filter `type = 'RT'`; wrapped in null-guard so the residual ~13% unmatched rows produce NULL instead of the placeholder hash                                                                                                         | Values unify (join predicate corrected; 96% of rows migrate from placeholder to real hash)                                          |
| `fct_grades_assignments.term_key` / `fct_grades_category.term_key` / `fct_grades_term.term_key` / `fct_staff_observations.term_key` / `dim_student_section_enrollments.term_key` / `dim_student_assessment_expectations.term_key` | `generate_surrogate_key([...6-tuple...])` — unwrapped; all unmatched LEFT JOIN rows hashed to the all-NULLs placeholder (`eb66153e09d138b38fc979bff5b437d5`), producing ~37M relationships-test orphans across the six consumers                                 | Same derivation, wrapped: `if(rt.code is not null, generate_surrogate_key([...6-tuple...]), cast(null as string))`. Grades facts also corrected: `fct_grades_assignments` filter `type='quarter'` → `'RT'`; `fct_grades_category` and `fct_grades_term` filter `'quarter'` → `'RT'`, join `storecode=rt.name` (was `rt.code`), `yearid=rt.powerschool_year_id` (was `powerschool_year_id - 1990`) | Null handling changes (rule 4) — plus predicate corrections that migrate grades-facts rows from placeholder-hash to real `term_key` |
| `dim_work_assignment_primary.work_assignment_primary_key`                                                                                                                                                                         | not present                                                                                                                                                                                                                                                      | `generate_surrogate_key(["item_id", "effective_date_start"])` — new SCD mart introduced in PR Batch B (#3687); narrow change-point SCD on `is_primary_position` per work assignment                                                                                                                                                                                                               | Structural add                                                                                                                      |
| `dim_staff_status.staff_status_key` / `dim_staff_status.staff_key`                                                                                                                                                                | composition `[en.employee_number, cp.effective_date_start]` and `[en.employee_number]` — but the `cp.associate_oid = en.adp_associate_id` inner join produced 0 matches in prod (16-char OID vs 9-char worker_id format mismatch), so all keys were absent       | Same compositions; join rekeyed to bridge through `worker_id__id_value` (`cp.worker_id = en.adp_associate_id`). Materialization goes from 0 rows to ~8.6K rows                                                                                                                                                                                                                                    | Structural add (effective — table was empty pre-fix; closes #3681)                                                                  |
| `dim_work_assignment_reporting_relationships.manager_staff_key`                                                                                                                                                                   | `if(mgr.employee_number is not null, generate_surrogate_key(["mgr.employee_number"]), null)` — but `cp.manager_associate_oid = mgr.adp_associate_id` join failed for all rows (same 16-char OID vs 9-char worker_id format mismatch), so column was 100% NULL    | Same composition; join rekeyed via a `workers` lookup CTE bridging `manager_associate_oid → worker_id__id_value → adp_associate_id`. `manager_staff_key` now populates with real values                                                                                                                                                                                                           | Structural add (effective — column was 100% NULL pre-fix)                                                                           |

#### Batch C — locations & regions (PR #TBD)

| Model.Column                                                                                                                                       | Before derivation                                                                                                               | After derivation                                                                                                                                             | Cause                                                                                    |
| -------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------- |
| `dim_regions.region_key`                                                                                                                           | `generate_surrogate_key(["region"])` (canonical short name)                                                                     | `generate_surrogate_key(["business_unit_code"])` after sourcing rows from `int_adp_workforce_now__workers__work_assignments__organizational_units__pivot`    | Composition change (rule 3) — hashes BU code instead of region name; new TAF row added   |
| `dim_work_assignment_locations.location_key`                                                                                                       | not present                                                                                                                     | passthrough of `int_adp_workforce_now__workers__work_assignments.location_key` (sourced from canonical master)                                               | Structural add (rule 5) — new FK; replaces 8 R9-dropped denormalized address columns     |
| `fct_behavioral_incidents.location_key`                                                                                                            | not present (row exposed degenerate `location` string instead)                                                                  | passthrough of `int_deanslist__incidents.location_key` (master-resolved via `deanslist_school_id`)                                                           | Structural add (rule 5) — replaces R9-dropped degenerate `location` string               |
| `dim_locations.location_key`                                                                                                                       | `generate_surrogate_key(["location_name"])` computed in mart                                                                    | passthrough from `stg_google_sheets__people__locations.location_key` (computed once on master)                                                               | No change — hash value identical (`MD5(location_name)` regardless of compute site)       |
| `fct_support_tickets.location_key`                                                                                                                 | `generate_surrogate_key(["cf.location"])` (Zendesk slug like `room9`) — produced placeholder hash for ~285K orphans             | passthrough from `int_zendesk__tickets__custom_fields_pivot.location_key` resolved via canonical master through alias-grain `int_people__location_crosswalk` | Values unify (rule 1) — orphan rows now resolve to real `dim_locations.location_key`     |
| `dim_staffing_positions.location_key`                                                                                                              | `generate_surrogate_key(["adp_location"])` (Seat Tracker typed string) — produced orphan hashes for ~478 rows                   | passthrough from `int_seat_tracker__snapshot.location_key`                                                                                                   | Values unify (rule 1)                                                                    |
| `fct_staff_observations.location_key`                                                                                                              | `generate_surrogate_key(["lc.location_clean_name"])` via `int_people__location_crosswalk` join                                  | passthrough from `int_schoolmint_grow__observations.location_key` (master-resolved via stable `grow_location_id`)                                            | Values unify (rule 1) — resolution path moved upstream; hash composition equivalent      |
| `fct_job_candidate_applications.shared_with_location_key`                                                                                          | `generate_surrogate_key(["school_shared_with"])` (raw SmartRecruiters string like `KIPP High School`) — orphans for ~358 rows   | passthrough from `int_people__location_crosswalk.location_key` joined on `school_shared_with`                                                                | Values unify (rule 1) — orphans resolve through new alias rows on the existing crosswalk |
| `dim_student_enrollments.location_key`                                                                                                             | `generate_surrogate_key(["loc.location_name"])` from `stg_people__locations` join — sentinel `999999` produced placeholder hash | passthrough from `stg_powerschool__schools.location_key` (LEFT JOIN to master; `999999` resolves to NULL naturally)                                          | Null handling change (rule 4) — sentinel rows now real NULL instead of placeholder hash  |
| `dim_course_sections.location_key`                                                                                                                 | same pattern as `dim_student_enrollments`                                                                                       | same                                                                                                                                                         | Null handling change (rule 4)                                                            |
| `dim_assessment_goals.location_key` / `dim_school_calendars.location_key` / `dim_terms.location_key` / `fct_student_attendance_daily.location_key` | `generate_surrogate_key(["loc.location_name"])` from `stg_people__locations` join                                               | passthrough from `stg_powerschool__schools.location_key` via `school_number` join                                                                            | No change — same hash values (consistency migration to unified PS pivot)                 |

#### Batch F — assessment & survey catalog (PR #TBD)

| Model.Column                                                                  | Before derivation                                                                                                                   | After derivation                                                                                                                                                                                                         | Cause                                                                                                                                          |
| ----------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `dim_assessment_administrations.assessment_administration_key`                | not present                                                                                                                         | `generate_surrogate_key(["assessment_type", "title", "subject_area", "scope", "module_code", "grade_level", "administered_date", "academic_year", "administration_round", "region", "season", "administration_window"])` | Structural add (rule 5) — new dim sitting between `dim_assessments` and the score facts at per-occurrence grain                                |
| `bridge_assessment_expectations_enrollment_scoped.assessment_expectation_key` | `generate_surrogate_key(["student_number", "assessment_id", "administered_at"])` on `dim_student_assessment_expectations` (deleted) | `generate_surrogate_key(["cc_dcid", "cc_source_relation", "assessment_id", "administered_at"])`                                                                                                                          | Structural add (rule 5) — new bridge replaces deleted `dim_student_assessment_expectations.student_assessment_expectation_key`                 |
| `bridge_assessment_expectations_student_scoped.assessment_expectation_key`    | `generate_surrogate_key(["student_number", "assessment_id", "administered_at"])` on `dim_student_assessment_expectations` (deleted) | `generate_surrogate_key(["powerschool_student_number", "assessment_id", "administered_at"])`                                                                                                                             | Structural add (rule 5) — new bridge replaces deleted `dim_student_assessment_expectations.student_assessment_expectation_key`; K-8 scope only |

#### Mart YAML audit — hash-input drift fixes (PR #3790)

| Model.Column                                                  | Before derivation                                                                                                                                                                                                                                                                                             | After derivation                                                                                                                                         | Cause                                                                                          |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `fct_survey_responses.survey_response_key`                    | `generate_surrogate_key(["survey_id", "survey_response_id", "respondent_identifier", "question_shortname"])` — collapsed Alchemer multi-question_id rows that share a shortname onto one PK, producing 2,458 PK collisions                                                                                    | `generate_surrogate_key(["survey_id", "survey_response_id", "respondent_identifier", "survey_question_id"])`                                             | Composition change (rule 3) — disambiguates Alchemer shortname collisions                      |
| `fct_student_attendance_daily.student_enrollment_key`         | `generate_surrogate_key(["ada.student_number", "ada._dbt_source_relation", "ada.academic_year", "enr.entrydate"])` — `ada._dbt_source_relation` produced `int_powerschool__ps_adaadm_daily_ctod` which never matched dim's `base_powerschool__student_enrollments` source-relation, creating 12.8M FK orphans | `generate_surrogate_key(["ada.student_number", "enr._dbt_source_relation", "ada.academic_year", "enr.entrydate"])`                                       | Composition change (rule 3) — aligns hash inputs with `dim_student_enrollments` (closes #3718) |
| `fct_student_attendance_interventions.student_enrollment_key` | `generate_surrogate_key(["ai.student_number", "ai._dbt_source_relation", "ai.academic_year", "enr.entrydate"])` — same source-relation drift; ~256K FK orphans                                                                                                                                                | `generate_surrogate_key(["ai.student_number", "enr._dbt_source_relation", "ai.academic_year", "enr.entrydate"])`                                         | Composition change (rule 3) — closes #3718                                                     |
| `fct_grades_term.student_section_enrollment_key`              | `generate_surrogate_key(["fg.cc_dcid", "fg._dbt_source_relation"])` — `fg._dbt_source_relation` produced `base_powerschool__final_grades` which never matched dim's `base_powerschool__course_enrollments` source-relation; ~262K FK orphans                                                                  | `generate_surrogate_key(["fg.cc_dcid", "replace(fg._dbt_source_relation, 'base_powerschool__final_grades', 'base_powerschool__course_enrollments')"])`   | Composition change (rule 3) — closes #3718                                                     |
| `dim_student_section_enrollments.student_enrollment_key`      | `generate_surrogate_key(["enr.student_number", "enr._dbt_source_relation", "cc.cc_academic_year", "enr.entrydate"])` — used `cc.cc_academic_year` instead of `enr.academic_year`, producing ~34K mismatches when the date-range join crossed an academic-year boundary                                        | `generate_surrogate_key(["enr.student_number", "enr._dbt_source_relation", "enr.academic_year", "enr.entrydate"])`                                       | Composition change (rule 3) — closes #3718                                                     |
| `dim_student_section_enrollments.course_section_key`          | `generate_surrogate_key(["cc.sections_dcid", "cc._dbt_source_relation"])` — `cc._dbt_source_relation` produced `base_powerschool__course_enrollments` which never matched parent dim's `base_powerschool__sections` source-relation; ~784K FK orphans                                                         | `generate_surrogate_key(["cc.sections_dcid", "replace(cc._dbt_source_relation, 'base_powerschool__course_enrollments', 'base_powerschool__sections')"])` | Composition change (rule 3) — closes #3718                                                     |

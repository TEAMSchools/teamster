...
| `cube-access-staff-data`         | Google group                             | Required to see any staff-domain cube members (staff, staff_work_history, staff observations, staff HR). Enforced solely via `queryRewrite` `isStaffMember` check (`startsWith("staff")`); not present in view access policies                                     | Pattern 3 (parallel to above) |

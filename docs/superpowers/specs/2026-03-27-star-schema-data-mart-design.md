...
| `dim_assessment_comparisons`              | Type 1 | one row per assessment x year x region                                      | Google Sheets state test comparison — external benchmarks (city, state, neighborhood schools percent proficient, total students). FK to `dim_regions` only. No FK to `dim_assessments` or `dim_assessment_administrations`: comparisons live at region x test x academic_year grain (no `grade_level`), coarser than `dim_assessments` (definition grain, includes `grade_level`) and `dim_assessment_administrations` (per-occurrence grain, includes `administered_date`). The FK is intentionally absent. Answers "how do we compare?" |

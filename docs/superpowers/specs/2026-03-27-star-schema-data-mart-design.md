...
| **Star schema**            | A fact table at the center, joined to dimension tables around it like points of a star. Simple, fast, and optimized for analytical queries.                                             | `fct_student_attendance_daily` at center, joined to `dim_student_enrollments` and `dim_dates`; `dim_locations`, `dim_regions`, and `dim_school_calendars` attributes reached via traversal.                                     |
...
| Fact/dim traversal           | Strict chain for non-conformed entity dims — every fact/dim FKs only to its finest-grain parent on each entity chain; broader entity context is reached via chain traversal. Facts split at grain boundaries rather than carry redundant FKs. Two exemptions: (1) conformed dims (`dim_dates`, `dim_terms`, `dim_regions`, `dim_locations`, `dim_school_calendars`) may be joined directly from each fact, and (2) role-playing (aliased) joins to any dim are permitted, including to non-conformed dims like `dim_staff` (teacher+observer, submitter+assignee, teammate+recruiter). | Matches Cube's transitive-join model; avoids unlabeled diamond subgraphs (which Cube docs explicitly flag as ambiguous) on entity chains. Conformed dims are universal aggregation/filter axes, and role-playing edges are semantically distinct by their aliases — Cube resolves both cleanly via explicit aliased joins |

...
### What the `/meta` check proves, and what it does not

`/meta` is generated by `CubeToMetaTransformer.compile()` — it serializes the
compiled data model and never touches the warehouse, verified 2026-09-23. So:

- **It proves the deployed model is the pinned model.** That is a real check.
  Piece 5 deploys by running a command against a tagged checkout, and a
  mis-deploy or a deploy that silently did not take is exactly the failure a
  human will not notice.
- **It cannot prove the sandbox data matches the model.** A dimension whose
  column is missing from the sandbox tables still appears in `/meta`, and the
  failure surfaces to MasterBorn at query time.

Calling it a drift gate oversells it. It is a deploy-integrity check, and the
data-side gap needs its own check.

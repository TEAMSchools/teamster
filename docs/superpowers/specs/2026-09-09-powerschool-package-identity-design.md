...
| `int_powerschool__gpa_term`, `int_powerschool__gpa_cumulative`, `int_powerschool__gpa_cumulative_year` | Join `stg_powerschool__students` on `studentid = id` and `stg_powerschool__schools` on `schoolid = school_number`. Emit `students_student_number` and the 3 school columns. `gpa_term_pivot` and `gpa_term_current` inherit them.                                                                                                                                       |

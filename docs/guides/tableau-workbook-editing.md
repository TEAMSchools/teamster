...
This is the mechanical companion to
[Tableau Permissions (People Data)](tableau-permissions.md), which describes
what the row-level-security calculations mean, and to the build reference at
...
Tags matter because `entra-ready` is the permissions inventory that
[the permissions guide](tableau-permissions.md) keys off. Losing it drops the
workbook out of the inventory with no error.

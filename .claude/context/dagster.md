...
- **`ASSET_FAILED_TO_MATERIALIZE` on a SUCCESS run is usually benign**: planned
  ...
  automation). `mcp__dagster-plus__get_run_logs` hides
...
`error.message`. Read it directly, and go to `get_run_compute_logs` for the full
dbt output the message points at.
...
logs, call `get_run_compute_logs(run_id=...)` — it resolves the opaque
compute-log key from the run's own `LogsCapturedEvent` and echoes it back as
`logKey`. Add `step_key` when the run captured logs for several step workers;
without it an ambiguous run returns the candidates instead of guessing. The

...
1. **IMPORTRANGE Sources** — the Connected Sheets extraction, one tab per dbt
   model, named exactly after the model. Refresh schedules are programmed here
   and only here.
...
dangerous case: the source sheet refreshes and looks correct, the Reports tab
keeps working, and the person reading it simply never sees the new column.
Nothing errors. See `docs/guides/google-sheets.md`'s table for exactly what to
do in each sheet for each kind of change (new model, column added/removed, tab
renamed, model retired).
...
| Model (IMPORTRANGE Sources tab name)                | Reports tab                  | What it holds                                                                                                                                                            |
| --------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `rpt_gsheets__gradebook_audit_all_weeks`            | `PS Full Calendar`           | Every school week of the current academic year, from `int_students__calendar_week` — not filtered to weeks already loaded into `U_EXPECTATIONS`.                         |
| `rpt_gsheets__gradebook_audit_current_expectations` | `Plugin Data Raw`            | Passthrough of `stg_powerschool__u_expectations` — what's actually live in PowerSchool right now, per row, with who/when it was created or changed.                      |
| `rpt_gsheets__gradebook_audit_template`             | `Template QW-Date Crosswalk` | One row per region × school level × quarter × week, counts as `W`/`H`/`F`/`S` columns — the week-number translation between Academics' sheet and PowerSchool's calendar. |

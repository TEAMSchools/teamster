...
| `zendesk_extract_corpus.py`                         | Extract the Zendesk Data-queue corpus (groups 21474460 + 31319068, two school years) to `.claude/scratch/zendesk/corpus.jsonl`; `--verify` asserts known counts. Reads raw Airbyte tables — `fct_support_tickets` drops ~33% of tickets and has no `group_id`. Output is PII (#4739).                                                |
| `zendesk_probe_seasonality.py`                      | Seasonality probe: category by week-offset-from-first-instructional-day, two-year-consistent cells, hand-tag worksheet (#4739)                                                                                                                                                                                                       |
| `zendesk_probe_reply_shape.py`                      | Reply-shape probe: classify first agent replies as link / attachment / pasted value / prose, emit slug worksheet. Link detection uses an `html_body`-derived flag because `plain_body` drops anchor hrefs (#4739)                                                                                                                    |
| `zendesk_probe_sample.py`                           | Partition probe: deterministic stratified sample for hand-labeling self-inflicted versus genuine load (#4739)                                                                                                                                                                                                                        |

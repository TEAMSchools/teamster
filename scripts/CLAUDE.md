...
| `gen-cambium-schema.py`                             | Regenerate `src/teamster/libraries/cambium/schema.py` (Pydantic model) from a real Cambium NJGPA summative record CSV header. Sample CSV is local-only (contains student PII, never committed); re-run only when Cambium changes the file layout.                                                                                    |

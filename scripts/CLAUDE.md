...
| `extract_quill_seta_ed_demographics.py`             | One-shot: de-identified demographics workbook for the Quill pilot research partner. Roster path, output dir, and key-file path are arguments — roster data never enters git. Needs PEP 723 deps, so plain `uv run scripts/...` works. See #4848.                                                                                     |

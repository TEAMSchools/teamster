...
| `build_maher_upload_file.py`                        | Build a Salesforce Data Loader insert CSV of new Maher Fund disbursements from a roster, deduped against a live `stg_kippadb__kipp_aid` export; appends the `[MAHER_FUND\|v1\|...]` marker tag. See the `building-maher-upload-files` skill for the runbook (#4277).             |

...
| `tableau_gpa_monitor/`                              | Build generators for the Cumulative GPA Monitor Tableau dashboard. `specs.py` holds the build as data (32 calcs, 12 parameters, 20 worksheets, the dashboard zone tree, 4 colour rules), all referenced by CAPTION so they re-apply to any baseline. `apply.py` applies the styling layer; `verify.py` diffs a rebuild against a reference; `extract_specs.py` recaptures specs after Desktop edits. Not standalone — see its README. |

...
- **SSH-tunnel suites take ~2-3 min** (loopback ssh-rsa servers in
  `test_ssh_paramiko_tunnel.py` / `test_resource_ssh_rekey.py`) — they exceed
  the 120s Bash timeout; run them with `run_in_background`. Do NOT chain
  `git stash` with a long-running verify in one command: a timeout can leave the
  changes stashed and the `stash pop` unrun. To confirm a failure is

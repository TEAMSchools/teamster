...
class DbtTableDepsAutomationCondition(BuiltinAutomationCondition[T_EntityKey]):
    ...
    def _get_dep_keys(
        ...
        dep_keys = asset_graph.get_ancestor_asset_keys(key)
        if self.allow_selection is not None:
...
test_table_automation_condition = (
...
test_view_automation_condition = (
    AutomationCondition.in_latest_time_window()
    & (
        AutomationCondition.newly_missing() | AutomationCondition.code_version_changed()
...
def test_foo():
    @asset(
        automation_condition=test_table_automation_condition,
    ...
    @asset(
        deps=[upstream_table],
        automation_condition=test_view_automation_condition,
        # automation_condition=test_table_automation_condition,

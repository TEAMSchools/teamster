with
    workers as (
        select associate_oid, effective_date_start, person_surrogate_key, person,
    ...
    flattened as (
        select
            ...
            /* transformations */
            cast(
                person.birthname.generationaffixcode.effectivedate as date
            ) as person__birth_name__generation_affix_code__effective_date,
            cast(
                person.birthname.qualificationaffixcode.effectivedate as date
            ) as person__birth_name__qualification_affix_code__effective_date,
            cast(
                person.legalname.generationaffixcode.effectivedate as date
            ) as person__legal_name__generation_affix_code__effective_date,
            cast(
                person.legalname.qualificationaffixcode.effectivedate as date
            ) as person__legal_name__qualification_affix_code__effective_date,
            cast(
                person.preferredname.generationaffixcode.effectivedate as date
            ) as person__preferred_name__generation_affix_code__effective_date,
            cast(
                person.preferredname.qualificationaffixcode.effectivedate as date
            ) as person__preferred_name__qualification_affix_code__effective_date,
            cast(
                person.ethnicitycode.effectivedate as date
            ) as person__ethnicity_code__effective_date,
            cast(
                person.gendercode.effectivedate as date
            ) as person__gender_code__effective_date,
            cast(
                person.genderselfidentitycode.effectivedate as date
            ) as person__gender_self_identity_code__effective_date,
            cast(
                person.highesteducationlevelcode.effectivedate as date
            ) as person__highest_education_level_code__effective_date,
            cast(
                person.legaladdress.countrysubdivisionlevel1.effectivedate as date
            ) as person__legal_address__country_subdivision_level_1__effective_date,
            cast(
                person.legaladdress.countrysubdivisionlevel2.effectivedate as date
            ) as person__legal_address__country_subdivision_level_2__effective_date,
            cast(
                person.legaladdress.namecode.effectivedate as date
            ) as person__legal_address__name_code__effective_date,
            cast(
                person.legaladdress.typecode.effectivedate as date
            ) as person__legal_address__type_code__effective_date,
            cast(
                person.maritalstatuscode.effectivedate as date
            ) as person__marital_status_code__effective_date,
            cast(
                person.militarystatuscode.effectivedate as date
            ) as person__military_status_code__effective_date,
            cast(
                person.preferredgenderpronouncode.effectivedate as date
            ) as person__preferred_gender_pronoun_code__effective_date,
            cast(
                person.racecode.effectivedate as date
            ) as person__race_code__effective_date,
            cast(
                person.racecode.identificationmethodcode.effectivedate as date
        ...
        from deduplicate
        where person_surrogate_key != person_surrogate_key_lag
    )

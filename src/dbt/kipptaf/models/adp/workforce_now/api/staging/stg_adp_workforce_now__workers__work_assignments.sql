with
    ...
    deduplicate as (
        select
            *,
            ...
            lag(work_assignments_surrogate_key, 1, '') over (
                partition by associate_oid order by effective_date_start asc
            ) as work_assignments_surrogate_key_lag,
        from workers
    ),
    ...
    flattened as (
        select
            ...
            /* transformations */
            cast(wa.hiredate as date) as hire_date,
            cast(wa.actualstartdate as date) as actual_start_date,
            cast(wa.senioritydate as date) as seniority_date,
            cast(wa.terminationdate as date) as termination_date,

            cast(
                wa.assignmentstatus.effectivedate as date
            ) as assignment_status__effective_date,
            cast(
                wa.payrollprocessingstatuscode.effectivedate as date
            ...
            cast(wa.jobcode.effectivedate as date) as job_code__effective_date,
            cast(
                wa.paycyclecode.effectivedate as date
            ) as pay_cycle_code__effective_date,
            cast(
                wa.workertypecode.effectivedate as date
            ) as worker_type_code__effective_date,

            cast(
                wa.assignmentstatus.statuscode.effectivedate as date
            ) as assignment_status__status_code__effective_date,
            cast(
                wa.assignmentstatus.reasoncode.effectivedate as date
            ) as assignment_status__reason_code__effective_date,
            cast(
                wa.homeworklocation.namecode.effectivedate as date
            ) as home_work_location__name_code__effective_date,
            cast(
                wa.standardhours.unitcode.effectivedate as date
            ) as standard_hours__unit_code__effective_date,
            cast(
                wa.wagelawcoverage.coveragecode.effectivedate as date
            ) as wage_law_coverage__coverage_code__effective_date,
            cast(
                wa.wagelawcoverage.wagelawnamecode.effectivedate as date
            ) as wage_law_coverage__wage_law_name_code__effective_date,

            cast(
                wa.baseremuneration.annualrateamount.namecode.effectivedate as date
            ) as base_remuneration__annual_rate_amount__name_code__effective_date,
            cast(
                wa.baseremuneration.payperiodrateamount.namecode.effectivedate as date
            ) as base_remuneration__pay_period_rate_amount__name_code__effective_date,
            cast(
                wa.baseremuneration.hourlyrateamount.namecode.effectivedate as date
            ) as base_remuneration__hourly_rate_amount__name_code__effective_date,
            cast(
                wa.baseremuneration.dailyrateamount.namecode.effectivedate as date
            ...
            cast(
                wa.homeworklocation.address.namecode.effectivedate as date
            ) as home_work_location__address__name_code__effective_date,
            cast(
                wa.homeworklocation.address.typecode.effectivedate as date
        ...
        where w.work_assignments_surrogate_key != work_assignments_surrogate_key_lag
    )

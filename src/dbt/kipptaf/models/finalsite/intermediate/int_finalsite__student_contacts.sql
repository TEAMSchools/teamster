...
-- contacts reporting (today KIPP Newark); its rows flow straight into
-- int_students__contacts' Finalsite branch, so a not-yet-cutover region (e.g.
-- Miami, still PowerSchool-sourced for contacts) must NOT be added here even
-- though its api layer is enabled.
with
    union_relations as (
        {{

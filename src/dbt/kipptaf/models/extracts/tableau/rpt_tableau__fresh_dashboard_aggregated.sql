...
select
    ...
    if(f.goal_name = 'Pending Offers', null, f.finalsite_id) as goal_name_value,
...
where
    ...
    and s.goal_name not in ('Deferred', 'Waitlisted')

union all

-- ever status: inquiries, applications, offers,
select
    s.academic_year,
    s.org,
    s.region,
...
where
    ...
    and s.goal_type in ('Inquiries', 'Applications', 'Offers')
...
where s.grouped_status_timeframe = 'Ever' and s.goal_type = 'Conversions'

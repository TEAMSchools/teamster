...
select
    ...
    sr.person__legal_name__given_name as `First name`,  /* legal name */
    sr.person__legal_name__family_name_1 as `Last name`,  /* legal name */
...
left join
    ...
    and sr.organizational_unit__home__department__name = tg.adp_department_home_name
...
left join
    ...
    and sr.organizational_unit__home__department__name = tg2.adp_department_home_name
...
where
    (
        sr.worker_type_code_name not in ('Intern', 'Part Time')
        or sr.worker_type_code_name is null
    )
    and coalesce(
        sr.worker_dates__termination_date, current_date('{{ var("local_timezone") }}')
    )

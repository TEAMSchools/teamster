...
select
    ...
    format_date('%m/%d/%Y', se.district_entry_date) as district_entry_date,
    format_date('%m/%d/%Y', se.district_entry_date) as school_entry_date,
    ...
    /* must check student_web_password from students table  */
    if(
    ...
    case
        when se.grade_level = 12 and se.fafsa = 'N'
        then se.fafsa
        when se.grade_level = 12 and se.overgrad_fafsa_opt_out = 'Yes'
        then 'E'
        when se.grade_level = 12 and se.salesforce_contact_has_fafsa = 'Yes'
        then 'C'
    end as s_stu_x__fafsa,

from {{ ref("int_extracts__student_enrollments") }} as se
left join  /* must check student_web_password from students table  */

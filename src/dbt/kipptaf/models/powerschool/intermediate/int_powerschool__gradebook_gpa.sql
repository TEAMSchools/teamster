...
select distinct
    ...
    if(
        sum(
            if(
                ...
                and f.quarter != 'Y1'
                and concat(f.region, f.school_level) not in ('CamdenES', 'NewarkES'),
                0,
                1
            )
        ) over (
            partition by
                f.quarter, f.student_number, f.course_number, f.section_or_period
        )
        = 0,
        1,
        0
    ...
    if(
        f.quarter != 'Y1' and f.quarter_course_percent_grade_that_matters > 100, 1, 0

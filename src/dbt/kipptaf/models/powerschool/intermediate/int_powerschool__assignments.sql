with
    assignments as (
        select
            ...
            if(
                gb.storecode = 'Q1', gb.storecode, concat('Q', right(gb.storecode, 1))
            ) as assign_quarter,
            if(
                if(
                    gb.storecode = 'Q1',
                    gb.storecode,
                    concat('Q', right(gb.storecode, 1))
                )
                in ('Q1', 'Q2'),
                'S1',
                'S2'
        ...
        left join
            {{ ref("int_powerschool__gradebook_assignmens") }} as a
        ...
        left join
            `teamster-332318.grangel.gradebook_audit_expectations` as aud
        ...
        where
            ...
            and enr.enroll_status = 0  -- only active student data is pulled
...
select
    ...
    safe_divide(assign_score_converted, assign_max_score) as assign_final_score_percent,
    assign_null_score,
    assign_scored,
...
from assignments
where
    assign_category_code not in ('H', 'Q')
    and es_exclude_concat not in ('NewarkESF', 'NewarkESS', 'CamdenESF', 'CamdenESS')

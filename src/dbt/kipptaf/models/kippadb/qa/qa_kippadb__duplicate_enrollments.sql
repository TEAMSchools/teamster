with
    enroll_scaffold as (
        select
            ...
            if
            (
                lag(e1.id, 1) over (partition by e1.student order by e1.start_date)
                = e2.id,
                1,
                0
            ) as dupe_check,
        ...
        inner join
            ...
            on e1.pursuing_degree_type = e2.pursuing_degree_type
        ...
        left join {{ ref("int_kippadb__roster") }} as r on e1.student = r.contact_id
        where
            e1.pursuing_degree_type

with
    contact as (
        select
            ...
            latest_state_financial_aid_app_date__c
            as latest_state_financial_aid_app_date,
            middle_school_attended__c as middle_school_attended,
            most_recent_college_enrollment_name__c
            as most_recent_college_enrollment_name,
            most_recent_college_enrollment_status__c
            ...
            coalesce(
                actual_hs_graduation_date__c, expected_hs_graduation__c
...
select
    *,
    ...
    if(
        extract(month from hs_graduation_date) < 10,
        extract(year from hs_graduation_date),
        extract(year from hs_graduation_date) + 1
    ) as graduation_year,

    case
        when college_match_display_gpa >= 3.50
        then '3.50+'
        when college_match_display_gpa >= 3.00
        then '3.00-3.49'
        when college_match_display_gpa >= 2.50
        then '2.50-2.99'
        when college_match_display_gpa >= 2.00
        then '2.00-2.49'
        when college_match_display_gpa < 2.00
        then '<2.00'
    end as college_match_gpa_band,
from contact

...
worker_person as (
    select
        ...
        work_assignment_assignment_status_long_name_prev
            as assignment_status_prev,
        ...
        work_assignment_full_time_equivalence_ratio
            as full_time_equivalence_ratio,
        ...
        work_assignment_custom_payroll_data_control
            as custom_payroll_data_control,
        ...
        organizational_unit_cost_number_assigned_name
            as cost_number_assigned_code,
        ...
        organizational_unit_department_assigned_name
            as department_assigned_code,
        ...
        COALESCE(group_group_long_name, group_group_short_name)
...
with_prestart as (
    ...
    where
        ...
        and (
            assignment_status_prev is null
            or assignment_status_prev = 'Terminated'
        )
),
...
crosswalk as (
    select
        ...
        COALESCE(
            ...
            SAFE_CAST(en.employee_number as string)
    ...
    inner join
        ...
        on
            wp.worker_id = en.adp_associate_id
            and en.is_active
    left join
    ...
    left join
        ...
        on
            en.employee_number = idps.employee_number
            and idps.is_active
),
...
with_manager as (
    select
        ...
        COALESCE(ph.preferred_name_given_name, ph.legal_name_given_name)
    ...
    left join
        {{ ref("stg_people__employee_numbers") }} as en
        on
            cw.report_to_worker_id = en.adp_associate_id
            and en.is_active
    left join
        {{ ref("stg_ldap__user_person") }} as ldap
        on en.employee_number = ldap.employee_number
...
left join
    ...
    on
        wm.powerschool_teacher_number = tgl.teachernumber

...
with
    scored_rows as (
        select exam_code, exam_grade, irregularity_code_1, irregularity_code_2,
        from
            {{ ref("stg_collegeboard__ap") }} unpivot (
                (exam_code, exam_grade, irregularity_code_1, irregularity_code_2)
    ...
    known_codes as (
        select code, `domain`,
        from {{ ref("stg_google_sheets__collegeboard__ap_codes") }}
    )

select 'Exam Codes' as expected_domain, f.code as missing_code,
from file_exam_codes as f
left join known_codes as k on f.code = k.code and k.`domain` = 'Exam Codes'
where k.code is null

union all

select 'Irregularity Scores' as expected_domain, f.code as missing_code,
from file_irreg_codes as f
left join known_codes as k on f.code = k.code and k.`domain` = 'Irregularity Scores'
where k.code is null

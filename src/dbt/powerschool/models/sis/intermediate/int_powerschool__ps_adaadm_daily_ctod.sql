...
select
    ...
    abs(attendancevalue - 1) as is_absent,

    if(att_code like 'T%', 0.67, attendancevalue) as is_present_weighted,
    if(att_code like 'T%', 1.0, 0.0) as is_tardy,
    if(att_code like 'T%', 0.0, 1.0) as is_ontime,
    if(att_code in ('OS', 'OSS', 'OSSP', 'SHI'), 1.0, 0.0) as is_oss,
    if(att_code in ('S', 'ISS'), 1.0, 0.0) as is_iss,
    if(att_code in ('OS', 'OSS', 'OSSP', 'S', 'ISS', 'SHI'), 1.0, 0.0) as is_suspended,
    if(
        att_code not in ('ISS', 'OSS', 'OS', 'OSSP', 'SHI'),
        abs(attendancevalue - 1),
        0.0

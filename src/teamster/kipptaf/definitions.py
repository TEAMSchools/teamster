...
defs = Definitions(
    ...
    resources={
        ...
        "gsheets": google_sheets.configured(
            config_from_files([f"{core_config_path}/resources/gsheets.yaml"])
        ),
        "sftp_blissbook": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_blissbook.yaml"])
        ),
        "sftp_clever": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_clever.yaml"])
        ),
        "sftp_coupa": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_coupa.yaml"])
        ),
        "sftp_deanslist": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_deanslist.yaml"])
        ),
        "sftp_egencia": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_egencia.yaml"])
        ),
        "sftp_illuminate": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_illuminate.yaml"])
        ),
        "sftp_kipptaf": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_kipptaf.yaml"])
        ),
        "sftp_littlesis": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_littlesis.yaml"])
        ),
        "sftp_razkids": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_razkids.yaml"])
        ),
        "sftp_read180": ssh_resource.configured(
            config_from_files([f"{local_config_path}/resources/sftp_read180.yaml"])
        ),
    },
)

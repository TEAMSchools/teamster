...
from teamster.libraries.dlt.zendesk import settings
from teamster.libraries.dlt.zendesk.credentials import (
    TZendeskCredentials,
...
def make_date_ranges(
    ...
    """Make tuples of (start, end) date ranges between the given `start` and `end`
    dates.
    The last range in the resulting list will be capped to the value of `end` argument
    so it may be smaller than `step`

    Example usage, create 1 week ranges between January 1st 2023 and today:
    >>> make_date_ranges(pendulum.DateTime(2023, 1, 1).as_tz('UTC'), pendulum.today(),
    timedelta(weeks=1))
    """
    ranges = []

...
from teamster.libraries.dlt.zendesk.chat import zendesk_chat
from teamster.libraries.dlt.zendesk.helpers import make_date_ranges
from teamster.libraries.dlt.zendesk.support import zendesk_support
from teamster.libraries.dlt.zendesk.talk import zendesk_talk
...
def load_support_with_pivoting():
    """
    Loads Zendesk Support data with pivoting. Simply done by setting the
    pivot_ticket_fields to true - default option. Loads only the base tables.
    """
    pipeline = dlt.pipeline(
...
def incremental_load_all_start_date():
    """
    Implements incremental load when possible to Support, Chat and Talk Endpoints. The
    default behaviour gets data since the last load time saved in dlt state or
    1st Jan 2000 if there has been no previous loading of the resource. With this
    setting, the sources will load data since the given data for all incremental
    endpoints.
    Last load time will still be updated.
    """

    # Choosing starting point for incremental load - optional, the default is the last
    # load time. If no last load time the start time will be the 1st day of the
    # millennium start time needs to be a pendulum datetime object
...
def incremental_load_with_backloading():
    """Backload historic data in ranges. In this method we load all tickets so far
    created since Jan 1st 2023 but one week at a time and then switch to incrementally
    loading new tickets. This can useful to reduce the potiential failure window when
    loading large amounts of historic data.
    ...
    ranges = make_date_ranges(
        start=min_start_date, end=max_end_date, step=timedelta(weeks=1)
    )

...
def process_ticket(
    ...
    """
    Helper function that processes a ticket object and returns a dictionary of ticket
    data.

    Args:
        ticket: The ticket dict object returned by a Zendesk API call.
        custom_fields: A dictionary containing all the custom fields available for
            tickets.
        pivot_custom_fields: A boolean indicating whether to pivot all custom fields or
            not.
            Defaults to True.
    ...
    for custom_field in ticket["custom_fields"]:
        if pivot_custom_fields:
            ...
            if field is None:
                logger.warning(
                    "Custom field with ID %s does not exist in fields state. It may "
                    "have been created after the pipeline run started.",
                    cus_field_id,
                )
...
def process_ticket_field(
    ...
    # if the id is new, add a new key to indicate that this is the initial value for
    # title
    # New dropdown options are added to existing field but existing options are not
    # changed

...
from teamster.libraries.dlt.zendesk.pipeline import settings
...
class ZendeskAPIClient:
    ...
    subdomain: str = ""
    url: str = ""
    headers: DictStrStr | None
    auth: tuple[str, TSecretValue] | None

    def __init__(
        self, credentials: TZendeskCredentials, url_prefix: str | None = None
    ) -> None:
        """
        Initializer for the API client which is then used to make API calls to the
        ZendeskAPI

        Args:
            credentials: ZendeskCredentials object which contains the necessary
                credentials to authenticate to ZendeskAPI
        """
        ...
        if isinstance(credentials, ZendeskCredentialsOAuth):
            ...
            raise TypeError(
                "Wrong credentials type provided to ZendeskAPIClient. The credentials "
                "need to be of type: ZendeskCredentialsOAuth, ZendeskCredentialsToken "
                "or ZendeskCredentialsEmailPass"
            )

        # If url_prefix is set it overrides the default API URL (e.g. chat api uses
        # zopim.com domain)
        if url_prefix:
            self.url = url_prefix
        else:
    ...
    def get_pages(
        ...
        """
        Makes a request to a paginated endpoint and returns a generator of data items
        per page.

        Args:
            endpoint: The url to the endpoint, e.g. /api/v2/calls
            data_point_name: The key which data items are nested under in the response
                object (e.g. calls)
        ...
        while get_url:
            ...
            if pagination == PaginationType.CURSOR:
                ...
                # See https://developer.zendesk.com/api-reference/ticketing/
                # ticket-management/incremental_exports/#json-format

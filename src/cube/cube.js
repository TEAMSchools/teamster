...
// cube-region-<slug>-* group slug → exact regions.region_name value in
// kipptaf_marts.dim_regions. BigQuery string equality is case-sensitive and
// the stored names are not uniformly cased (TAF is all-caps, the rest are
// title-case), so a mechanical transform of the slug can't match them all.
// An unmapped slug yields undefined → empty values → default deny (safe).
const REGION_NAMES = {
  camden: "Camden",
  miami: "Miami",
  newark: "Newark",
  ...
  queryRewrite: (query, { securityContext }) => {
    ...
    if (networkGroup) {
      ...
      locationFilter = {
        ...
        operator: "equals",
        values: regionName ? [regionName] : [], // unmapped slug → default deny
      };
    } else if (schoolGroup) {
